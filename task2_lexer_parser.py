"""Завдання. 
Додати нові типи токенів для операцій множення MUL, ділення DIV та дужок, 
які відкривають LPAREN та закривають RPAREN частину арифметичного виразу.
Модифікувати метод get_next_token класу Lexer так, щоб він розпізнавав нові символи.
Додати метод для обробки чисел та виразів у дужках, метод для обробки множення та ділення.
Модифікувати метод visit_BinOp у класі Interpreter так, щоб він міг обробляти операції множення та ділення.
"""
...
class TokenType:            #додано атрибути для представлення операцій множення та ділення, і лівої та правої дужок
    INTEGER = "INTEGER"
    PLUS = "PLUS"
    MINUS = "MINUS"
    MUL = "MUL"
...
class Parser:
    ...
    def factor(self):
        """метод для обробки виразів у дужках"""
    ...
    def term(self):
        """Парсер для арифметичних операцій множення та ділення"""
        node = self.factor()
...
class Interpreter:
    ...
    def visit_BinOp(self, node):                            #додано інструкції для операцій множення та ділення
